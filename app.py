...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'adega-secret-default-fallback-123')
...
class DashboardState(db.Model):
    ...
    agua = db.Column(db.Float, default=0.0)
    energia = db.Column(db.Float, default=0.0)
    combustivel = db.Column(db.Float, default=0.0)
...
def get_data():
    ...
    return jsonify({
        ...
        "agua": state.agua, "energia": state.energia, "combustivel": state.combustivel,
...
def save_data():
    ...
    state.agua = data.get('agua', 0)
    state.energia = data.get('energia', 0)
    state.combustivel = data.get('combustivel', 0)
    state.aluguel = data.get('aluguel', 0)
...
def clear_data():
    DashboardState.query.filter_by(user_id=current_user.id).update({
        ...
        "agua": 0, "energia": 0, "combustivel": 0, "aluguel": 0
