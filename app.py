...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'adega-secret-key-12345' # Em produção, use algo aleatório

# Configuração do Banco de Dados SQLite
db_path = os.path.join(os.path.dirname(__file__), 'adega.db')
...
with app.app_context():
    db.create_all()
    if not DashboardState.query.first():
        db.session.add(DashboardState())
    db.session.commit()
...
@login_required
def get_data():
    state = DashboardState.query.first()
    return jsonify({
        "fat": state.faturamento, "socios": state.socios,
...
def save_data():
    data = request.json
    state = DashboardState.query.first()
    state.faturamento = data.get('fat', 0)
